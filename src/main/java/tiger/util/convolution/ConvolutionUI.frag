...
void main(void) {
    ...
    uvec4 B = texture2D(texture, texCoord + vec2(0, -heightInversed));
    //vec4 C = texture2D(texture, texCoord + vec2(widthInversed, -heightInversed));
    uvec4 D = texture2D(texture, texCoord + vec2(-widthInversed, 0));
    uvec4 E = texture2D(texture, texCoord + vec2(widthInversed, 0));
    //vec4 F = texture2D(texture, texCoord + vec2(-widthInversed, heightInversed));
    uvec4 G = texture2D(texture, texCoord + vec2(0, heightInversed));
